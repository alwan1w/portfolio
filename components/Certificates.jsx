...
export default function Certificates() {
  ...
  return (
    <section id="certificates" className="min-h-screen py-20 bg-[#0f0f0f] relative overflow-hidden">
      ...
      <div className="relative z-10 px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <div className="mb-16 text-center">
          <span className="font-mono text-sm text-emerald-400"> Credentials</span>
          <h2 className="mt-2 mb-4 text-5xl font-bold">
            <span className="text-white">Certificates & </span>
            <span className="text-transparent bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text">Achievements</span>
          </h2>
        ...
        <div className="grid gap-6 mb-12 md:grid-cols-2">
          {certificates.map((cert, index) => (
            <div
              ...
              className={`group relative bg-[#0a0a0a] border rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2 cursor-pointer ${colorMap[cert.color]}`}
            >
              {cert.featured && (
              ...
              <div className="relative w-full overflow-hidden h-52">
                ...
                <div className="absolute inset-0 flex items-center justify-center transition-opacity opacity-0 bg-black/50 group-hover:opacity-100">
                  <span className="px-4 py-2 font-mono text-sm text-white border border-white rounded-full">
                    Klik untuk perbesar
                  </span>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between">
                  <span className={`px-3 py-1 font-mono text-xs border rounded-full ${badgeColorMap[cert.color]}`}>
                    {cert.date}
                  </span>
                </div>
        ...
        <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
          {achievements.map((stat, index) => (
            <div
              key={index}
              className="relative group text-center p-8 bg-[#0a0a0a] border border-gray-800 rounded-2xl hover:border-emerald-500/50 transition-all hover:scale-105"
            >
              <div className="absolute inset-0 transition-opacity opacity-0 bg-gradient-to-br from-emerald-500/5 to-cyan-500/5 rounded-2xl group-hover:opacity-100"></div>
      ...
      {selectedCert && (
        ...
        >
          ...
          >
            <button
              onClick={() => setSelectedCert(null)}
              className="absolute z-10 p-2 transition-colors bg-gray-900 border border-gray-800 rounded-full top-4 right-4 hover:border-red-500"
            >
            ...
            <div className="p-6">
              <h3 className="mb-1 text-xl font-bold text-white">{selectedCert.title}</h3>
              <p className="font-mono text-sm text-gray-400">{selectedCert.issuer}</p>
              <p className="mt-1 font-mono text-xs text-gray-600">{selectedCert.credentialId}</p>
            </div>
          </div>
        </div>
      )}
